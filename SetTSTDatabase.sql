...
BEGIN
   ...
   DECLARE @ExpectedErrorLikeMessage   nvarchar(2048) 
   DECLARE @LikeEscapeCharacter        char
   DECLARE @ExpectedErrorMode          int
   ...
   SET @ExpectedErrorMode = 0
   SELECT 
      @ExpectedErrorNumber       = ExpectedErrorNumber      ,
      @ExpectedErrorMessage      = ExpectedErrorMessage     ,
      @ExpectedErrorProcedure    = ExpectedErrorProcedure   ,
      @ExpectedErrorLikeMessage  = ExpectedErrorLikeMessage ,
      @LikeEscapeCharacter       = LikeEscapeCharacter

   FROM #Tmp_CrtSessionInfo

   --@ExpectedErrorLikeMessage  nvarchar(2048) = NULL,
   --@LikeEscapeCharacter       char = NULL

   IF ( (@ExpectedErrorNumber IS NOT NULL) OR (@ExpectedErrorMessage IS NOT NULL) OR (@ExpectedErrorProcedure IS NOT NULL) OR (@ExpectedErrorLikeMessage IS NOT NULL))
   BEGIN
      IF (      (@Catch_ErrorNumber     = @ExpectedErrorNumber                                     OR @ExpectedErrorNumber IS NULL      )
            ...
            AND (@Catch_ErrorMessage LIKE @ExpectedErrorLikeMessage ESCAPE @LikeEscapeCharacter    OR @ExpectedErrorLikeMessage IS NULL )
            AND (@Catch_ErrorProcedure  = @ExpectedErrorProcedure                                  OR @ExpectedErrorProcedure IS NULL   ))
      BEGIN
         SET @ExpectedErrorMode = 1
      END
      ELSE
      BEGIN
         SET @ExpectedErrorMode = 2
      END
   END
   ...
   IF (@ExpectedErrorMode = 1)
   BEGIN
      -- The expected error was a match for the actual error.
   ...
   BEGIN
      ...
      IF(@ExpectedErrorMode = 0)
      BEGIN
         -- There was no expected error registered.
      ...
      ELSE
      BEGIN
         -- There was an expected error registered but it did not match the actual error.
         SET @ErrorMessage =  'An error different than the expected error occured during the execution of the test procedure ''' + @FullSprocName + '''. ' +
                              'Expected Error Number: ' + ISNULL(CAST(@ExpectedErrorNumber AS varchar), '<not set>') + ', ' +
                              'Actual Error Number: ' + CAST(ERROR_NUMBER() AS varchar)  + ', ' +
                              'Expected Error Message: ' + ISNULL('''' + @ExpectedErrorMessage + '''', '<not set>') + ', ' +
                              'Expected Error Like Message: ' + ISNULL('''' + @ExpectedErrorLikeMessage + '''', '<not set>') + ', ' +
                              'LikeEscapeCharacter: ' + ISNULL('''' + CAST(@LikeEscapeCharacter AS varchar) + '''', '<not set>') + ', ' +
                              'Actual Error Message: ''' + ERROR_MESSAGE() + ''', ' +
                              'Expected Error Procedure: ' + ISNULL(@ExpectedErrorProcedure, '<not set>') + ', ' +
                              'Actual Error Procedure: ' + ISNULL(ERROR_PROCEDURE(), 'N/A') + ', ' +
                              'Actual Error Line: ' + CAST(ERROR_LINE() AS varchar)
      END
...
CREATE PROCEDURE Internal.GetExpectedErrorInfo
   ...
   @ExpectedErrorInfo            nvarchar(max) OUT 
AS
BEGIN
   ...
   DECLARE @ExpectedErrorLikeMessage      nvarchar(2048)
   DECLARE @LikeEscapeCharacter           char
   DECLARE @ExpectedErrorMessageString    nvarchar(max)
   ...
   IF (     (@ExpectedErrorNumber IS NOT NULL) 
         ...
         OR (@ExpectedErrorLikeMessage IS NOT NULL))
   BEGIN
      ...
      SET @ExpectedErrorMessageString = 'Message: N/A'
      IF (@ExpectedErrorMessage IS NOT NULL OR @ExpectedErrorLikeMessage IS NOT NULL)
      BEGIN
         IF (@ExpectedErrorMessage IS NOT NULL)
         BEGIN
            SET @ExpectedErrorMessageString = 'Message: ''' + @ExpectedErrorMessage + ''''
         END
         ELSE
         BEGIN
            SET @ExpectedErrorMessageString = 'Message [LIKE'
            IF (@LikeEscapeCharacter IS NOT NULL)
            BEGIN
               SET @ExpectedErrorMessageString = @ExpectedErrorMessageString + ', ESCAPE=''' + @LikeEscapeCharacter + ''''
            END
            SET @ExpectedErrorMessageString = @ExpectedErrorMessageString + ']: ''' + @ExpectedErrorLikeMessage + ''''
         END
      END

      SET @ExpectedErrorInfo = @ExpectedErrorInfo + ' ' + @ExpectedErrorMessageString
   END
...
BEGIN
   ...
   BEGIN
      ...
      SELECT 
         ...
         @TestDatabaseName    = TestSession.DatabaseName 
      ...
      ELSE SET @TestSessionStatusString = 'Failed'

      SELECT @TestServerName = @@SERVERNAME

      SET @ResultMessage = '<TST status="' + @TestSessionStatusString + '"'

      IF (@NoTimestamp=0)
      BEGIN
         SET @ResultMessage = @ResultMessage + 
            ' SQLServer="' + Internal.SFN_EscapeForXml(@TestServerName) + '"' +
            ' SQLDatabase="' + Internal.SFN_EscapeForXml(@TestDatabaseName) + '"' +
...
-- It will print all the results of the current test session. 
-- =======================================================================
CREATE PROCEDURE Internal.PrintResults
   @TestSessionId    int,         -- Identifies the test session.
   @ResultsFormat    varchar(10), -- Indicates if the format in which the results will be printed.
                                  -- See the coments at the begining of the file under section 'Results Format'
   @NoTimestamp      bit = 0,     -- Indicates that no timestamp or duration info should be printed in results output
...
BEGIN
   ...
   IF (@DataTypeName = 'time'             ) RETURN 1
   IF (@DataTypeName = 'date'             ) RETURN 1
   IF (@DataTypeName = 'datetime2'        ) RETURN 1
   IF (@DataTypeName = 'datetimeoffset'   ) RETURN 1
...
BEGIN
   ...
   ELSE IF (@DataTypeName = 'time'             )    BEGIN SET @ConvertStyle = ', 114'; END
   ELSE IF (@DataTypeName = 'date'             )    BEGIN SET @ConvertStyle = ', 101'; END
   ELSE IF (@DataTypeName = 'datetime2'        )    BEGIN SET @ConvertStyle = ', 121'; END
   ELSE IF (@DataTypeName = 'datetimeoffset'   )    BEGIN SET @ConvertStyle = ', 121'; END
...
--       time                       Date and Time              DT
--       date                       Date and Time              DT
--       datetime2                  Date and Time              DT
--       datetimeoffset             Date and Time              DT
...
BEGIN
   ...
   ELSE  IF (@BaseType = 'time'             ) BEGIN SET @DataTypeFamily = 'DT'; SET @StringValue = CONVERT(nvarchar(max), @SqlVariant, 114 ); END
   ELSE  IF (@BaseType = 'date'             ) BEGIN SET @DataTypeFamily = 'DT'; SET @StringValue = CONVERT(nvarchar(max), @SqlVariant, 101 ); END
   ELSE  IF (@BaseType = 'datetime2'        ) BEGIN SET @DataTypeFamily = 'DT'; SET @StringValue = CONVERT(nvarchar(max), @SqlVariant, 121 ); END
   ELSE  IF (@BaseType = 'datetimeoffset'   ) BEGIN SET @DataTypeFamily = 'DT'; SET @StringValue = CONVERT(nvarchar(max), @SqlVariant, 121 ); END
...
BEGIN
   ...
   IF (@ExpectedErrorMessage IS NOT NULL AND @ExpectedErrorLikeMessage IS NOT NULL)
   BEGIN
      SET @ErrorMessage = 'Invalid call to Assert.RegisterExpectedError [' + @ContextMessage + ']. @ExpectedErrorMessage and @ExpectedErrorLikeMessage cannot both be specified.'
      EXEC Internal.LogErrorMessageAndRaiseError @ErrorMessage
   END

   IF (@LikeEscapeCharacter IS NOT NULL AND @ExpectedErrorLikeMessage IS NULL)
   BEGIN
      SET @ErrorMessage = 'Invalid call to Assert.RegisterExpectedError [' + @ContextMessage + ']. @LikeEscapeCharacter can only be specified when @ExpectedErrorLikeMessage is also specified.'
      EXEC Internal.LogErrorMessageAndRaiseError @ErrorMessage
   END
...
-- =======================================================================
-- PROCEDURE: Assert.NVarCharMaxEquals
-- Can be called by the test procedures to verify that 
-- two nvarchar(max) values are equal. 
-- Note: NULL is invalid for @ExpectedValue. If Assert.Equals is
--       called with NULL for @ExpectedValue then it will fail with 
--       an ERROR. Use Assert.IsNull instead.
-- Result map:
...
-- If failes it will record an entry in TestLog and raise an error.
-- =======================================================================
CREATE PROCEDURE Assert.NVarCharMaxEquals
   @ContextMessage      nvarchar(1000),
...
BEGIN
   ...
   IF (@ExpectedValue IS NULL )
   BEGIN
      SET @Message = 'Invalid call to Assert.NVarCharMaxEquals. [' + @ContextMessage + '] @ExpectedValue cannot be NULL. Use Assert.IsNull instead.'
      EXEC Internal.LogErrorMessageAndRaiseError @Message
      RETURN
   END
   ...
   IF (@ActualValue IS NULL )
   BEGIN
      SET @Message = 'Assert.NVarCharMaxEquals failed. [' + @ContextMessage + '] Actual value is NULL'
      EXEC Assert.Fail @Message
   END

   IF (@ActualValue = @ExpectedValue)
   BEGIN
      SET @Message = 'Assert.NVarCharMaxEquals passed. [' + @ContextMessage + '] Test value: ''' + @ExpectedValue + '''. Actual value: ''' + @ActualValue + ''''
      EXEC Assert.Pass @Message
      RETURN
   END

   SET @Message = 'Assert.NVarCharMaxEquals failed. [' + @ContextMessage + '] Test value: ''' + @ExpectedValue + '''. Actual value: ''' + @ActualValue + ''''
   EXEC Assert.Fail @Message
...
-- =======================================================================
-- PROCEDURE: Assert.NVarCharMaxNotEquals
-- Can be called by the test procedures to verify that 
-- two nvarchar(max) values are equal. 
-- Note: NULL is invalid for @ExpectedValue. If Assert.Equals is
--       called with NULL for @ExpectedValue then it will fail with 
...
-- If failes it will record an entry in TestLog and raise an error.
-- =======================================================================
CREATE PROCEDURE Assert.NVarCharMaxNotEquals
   @ContextMessage      nvarchar(1000),
   @ExpectedNotValue    nvarchar(max),
   @ActualValue         nvarchar(max)
AS
BEGIN
   ...
   IF (@ExpectedNotValue IS NULL )
   BEGIN
      SET @Message = 'Invalid call to Assert.NVarCharMaxNotEquals. [' + @ContextMessage + '] @ExpectedNotValue cannot be NULL. Use Assert.IsNotNull instead.'
      EXEC Internal.LogErrorMessageAndRaiseError @Message
      RETURN
   END
   ...
   IF (@ActualValue IS NULL )
   BEGIN
      SET @Message = 'Assert.NVarCharMaxNotEquals failed. [' + @ContextMessage + '] Actual value is NULL'
      EXEC Assert.Fail @Message
   END

   IF (@ActualValue != @ExpectedNotValue)
   BEGIN
      SET @Message = 'Assert.NVarCharMaxNotEquals passed. [' + @ContextMessage + '] Test value: ''' + @ExpectedNotValue + '''. Actual value: ''' + @ActualValue + ''''
      EXEC Assert.Pass @Message
      RETURN
   END

   SET @Message = 'Assert.NVarCharMaxNotEquals failed. [' + @ContextMessage + '] Test value: ''' + @ExpectedNotValue + '''. Actual value: ''' + @ActualValue + ''''
   EXEC Assert.Fail @Message
...
CREATE PROCEDURE Assert.IsLike
   @ContextMessage      nvarchar(1000),
   @ExpectedLikeValue   nvarchar(4000),
   @ActualValue         nvarchar(max),
...
CREATE PROCEDURE Assert.IsNotLike
   @ContextMessage         nvarchar(1000),
   @ExpectedNotLikeValue   nvarchar(4000),
   @ActualValue            nvarchar(max),
   @EscapeCharacter        char = NULL
AS
BEGIN
...
BEGIN
   ...
   BEGIN
      CREATE TABLE #Tmp_CrtSessionInfo (
         ...
         ExpectedErrorLikeMessage      nvarchar(2048),
         LikeEscapeCharacter           char,
